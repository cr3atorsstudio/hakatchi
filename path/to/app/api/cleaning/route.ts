...
export async function POST(request: Request) {
  ...
  if (!grave_id || effort_level === undefined) {
    return NextResponse.json({ error: 'grave_id and effort_level are required' }, { status: 400 })
  }

  if (effort_level < 1 || effort_level > 10) {
    return NextResponse.json({ error: 'effort_level must be between 1 and 10' }, { status: 400 })
  }
